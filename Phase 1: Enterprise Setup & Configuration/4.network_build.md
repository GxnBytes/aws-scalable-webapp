...
3. Selected **"Add a new forest"** and entered `corp.skynet-dc.com` as the domain name.  
   This will act as the main domain for connecting all workstations.

4. Clicked **Next**. On the **Domain Controller Options** screen, I left the defaults selected.  
   For the **DSRM (Directory Services Restore Mode)** password, I used the same one set earlier for the administrator account.

5. Clicked **Next** twice.

6. Left **CORP** as the **NetBIOS** name and clicked **Next**.

7. Continued clicking **Next** until I reached the **Prerequisites Check**.  
   Allowed it to apply settings automatically, then clicked **Install**.

8. Once the installation completed, the window closed and the server restarted automatically.
